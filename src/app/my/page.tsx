...
export const metadata = { title: "My filings | Adhikaar" };
export const dynamic = "force-dynamic";

export default async function MyFilingsPage() {
  const { uid } = await requireSession("/my");
      ...
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <h1 className="font-display font-bold text-3xl" style={{ color: "var(--ink)" }}>
            Everything you have filed, kept together
          </h1>
        </div>
        <Link
          href="/start"
      ...
      {cases.length === 0 ? (
        <div className="border-2 border-dashed p-12 text-center" style={{ borderColor: "var(--rule)" }}>
          ...
          <p className="mb-6" style={{ color: "var(--ink-soft)" }}>
            Describe what went wrong and the app will work out who is legally required to
            answer you, before anything is drafted.
          </p>
          <Link href="/start" className="underline font-medium" style={{ color: "var(--seal-deep)" }}>
          ...
          <table className="ledger w-full text-sm">
            ...
            <tbody>
              {cases.map((c) => {
                ...
                const overdue = Boolean(nextDeadline && isOverdue(nextDeadline));
                return (
                  <tr key={c.id} style={overdue ? { background: "var(--brick-tint)" } : undefined}>
                    <td className="p-3 align-top">
                      <Link href={`/my/${c.id}`} className="font-medium underline" style={{ color: "var(--ink)" }}>
                    ...
                    <td className="p-3 align-top text-sm">
                      {nextDeadline ? (
                        <span style={{ color: overdue ? "var(--brick)" : "var(--ink-soft)" }}>
                          {nextDeadline.label},{" "}
                          {overdue
                        ...
                        <span style={{ color: "var(--ink-faint)" }}>Nothing pending</span>
                      )}
                    </td>
                  </tr>
                );
