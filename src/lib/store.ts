...
// persistence. Everything else in the app imports from here and only
...
export async function updateCase(
  id: string,
  patch: Parameters<CaseStore["updateCase"]>[1]
) {
  return getStore().updateCase(id, patch);
}

export async function deleteCase(id: string) {
  return getStore().deleteCase(id);
}
